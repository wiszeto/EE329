...
int main(void) {
  ...
  while (1) {
    ...
    if (display_state) {
      ...
      lcd_set_cursor_position(1, 0); // set cursor to second row, first column
      str_write("TIME = ");
      elapsed_time = elapsed_time / 2250;

      if (elapsed_time < 1000) {
        char str[4]; // maximum number of digits for a 10-bit number is 4
        ...
        char str[5]; // maximum number of digits for a 10-bit number is 5
                     // (including the decimal point)
      ...
      while (1) {
        if (GPIOC->IDR & GPIO_IDR_ID13) {
          ...
          delay_us(100);
          str_write(
              "PUSH SW TO TRIG "); // write *=SET #=GO --:-- to second line
          delay_us(10000000);
          break;
        }
      }
