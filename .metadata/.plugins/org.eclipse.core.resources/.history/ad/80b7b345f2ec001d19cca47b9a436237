...
void DAC_write(uint32_t dig_val) {
	SPI_select_slave(); // Select the slave device
...
void SPI_select_slave(void) {
    SPI1->CR1 &= ~SPI_CR1_SSI; // Clear the SSI bit, setting the NSS pin low (selected)
}

void SPI_deselect_slave(void) {
    SPI1->CR1 |= SPI_CR1_SSI; // Set the SSI bit, setting the NSS pin high (not selected)
}
